...
# Plot 2: Total PM2.5 Emissions for Baltimore City, Maryland (fips == "24510") vs year (1999 - 2008)
plotdata <- NEI %>% filter(fips == "24510") %>% group_by(year) %>% summarise(sum(Emissions))
...
with(plotdata, 
     {
       ...
       plot(`sum(Emissions)`~year, type = "l", col="black", main="Total PM2.5 Emissions (in tons) for Baltimore City vs Year")
